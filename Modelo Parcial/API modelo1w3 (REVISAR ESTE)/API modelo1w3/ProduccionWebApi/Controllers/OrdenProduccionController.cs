...
namespace ProduccionAPI.Controllers
{
    ...
    public class OrdenProduccionController : ControllerBase
    {
        ...
        //url
        [HttpGet("ordenes")] // sevidor.puerto/api/OrdenProduccion/ordenes?fecha = ''& estado = ''
        public IActionResult GetOrdenes([FromQuery]DateTime? fecha, [FromQuery]string? estado)
        {
            try
            {
                ...
                if (lst.Count == 0)
                    return NotFound("No se encontraron ordenes de producción, para los filtros indicados.");
                else
                    return Ok(lst);
            }
            catch (Exception)
